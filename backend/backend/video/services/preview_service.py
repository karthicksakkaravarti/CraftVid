...
def generate_preview(
    ...
    try:
        ...
        # Set duration and size for the image clip
        image_clip = image_clip.with_duration(duration)
        # Ensure 16:9 aspect ratio (1280x720)
        image_clip = image_clip.resized(width=1280, height=720)
        ...
        relative_path = os.path.join(
            "previews", str(screen.workspace.id), str(screen.script.id) if screen.script else "", output_filename
        )
